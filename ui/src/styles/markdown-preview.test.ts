...
describe("markdown preview styles", () => {
  it("keeps the preview dialog canvas unified", async () => {
    const css = await readFile(path.join(process.cwd(), "src/styles/components.css"), "utf8");
  ...
  it("keeps the sidebar reader shell in sidebar.css", async () => {
    const css = await readFile(path.join(process.cwd(), "src/styles/chat/sidebar.css"), "utf8");
